      ...
      <q-card style="min-width: 350px; margin: 0">
        <q-card-section>
          <div class="iconoAprendiz">
            <img
              src="https://cdn-icons-png.flaticon.com/256/72/72648.png"
              alt=""
            />
          </div>
          <div class="text">
...
async function agregarAprendiz() {
  let res;
  if (p.value == false) {
    res = await useAprendiz.registrarAprendiz(
      nombre.value,
      telefono.value,
      documento.value,
      email.value,
...
async function Activar(id) {
  ...
  try {
    inf = await axios.put(
      `https://aprendices-asistencia-bd-3.onrender.com/api/Aprendiz/Desactivar/${id}`
    );
    traer();
  } catch (error) {
    console.log(error);
...
async function Desactivar(id) {
  ...
  try {
    inf = await axios.put(
      `https://aprendices-asistencia-bd-3.onrender.com/api/Aprendiz/Activar/${id}`
    );
    traer();
  } catch (error) {
    console.log(error);
